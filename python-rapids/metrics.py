...
def create_metrics(df, y_test, threshold_cols):
    ...
    for threshold_col in threshold_cols:
        conf_matrix = confusion_matrix(y_test.astype("int32"), df[threshold_col].astype("int32"))
...
def save_metrics(all_accuracy_list: list,
    ...
    accuracy_df = cudf.DataFrame(all_accuracy_list, columns=threshold_col_names)
    f1_score_df = cudf.DataFrame(all_f1_score_list, columns=threshold_col_names)
    precision_df = cudf.DataFrame(all_precision_list, columns=threshold_col_names)
    sensitivity_df = cudf.DataFrame(all_sensitivity_list, columns=threshold_col_names)
    specificity_df = cudf.DataFrame(all_specificity_list, columns=threshold_col_names)
...
def save_prediction_df(df: cudf.DataFrame,
                       dataset_name: str,
                       path: str) -> None:
...
def save_best_model_parameters(best_params_dict: dict,
    ...
    df = cudf.DataFrame.from_dict(tmp_dict)
    df.to_csv(os.path.join(path, dataset_name), sep=',', index=False)